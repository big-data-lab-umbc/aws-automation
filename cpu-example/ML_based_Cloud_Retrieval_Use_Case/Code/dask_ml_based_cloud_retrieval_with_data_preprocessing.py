...
def ml_process(scheduler_address):
    ...
    da_np_caliop = da.from_array(np_caliop, chunks='auto')
    #print(da_np_caliop.compute())     
    ...
    da_df_vnp02 = dd.from_pandas(df_vnp02, npartitions=1)
    print(da_df_vnp02.head())     

    # separate the data set into two parts (training 70% and testing 30%)
    testing_size = 0.30
    seed = 7
    da_x = da_df_vnp02.to_dask_array(lengths=True)
    train_X, test_X, train_Y, test_Y = train_test_split(da_x,da_np_caliop,test_size=testing_size,random_state=seed)

    # check the size of training and testing parts
    print(train_X.shape,train_Y.shape)
    print(test_X.shape, test_Y.shape)

    # da_enc = preprocessing.LabelEncoder()
    # train_Y_encoded = da_enc.fit_transform(train_Y.ravel())
    # test_Y_encoded = da_enc.fit_transform(test_Y.ravel())
    ...
    print(accuracy_score(train_Y, da.from_array(train_predict_Y, chunks='auto')))

    print('Testing Data Performance')
    test_predict_Y = rfforest.predict(test_X)
    test_predict_YP = rfforest.predict_proba(test_X)
    print(accuracy_score(test_Y, da.from_array(test_predict_Y, chunks='auto')))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print("Usage: /usr/bin/python3.6 dask_ml_based_cloud_retrieval_with_data_preprocessing.py <tcp://172.17.x.x:8786>", file=sys.stderr)
        sys.exit(-1)

    dask_scheduler_address = str(sys.argv[1])

    start = time.time()
    ml_process(dask_scheduler_address)
    end = time.time()
